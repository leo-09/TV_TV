...
@property (nonatomic, strong) NSMutableArray<UIButton *> *itemBtnArr;   // 标题的Button组
...
- (void)layoutSubviews {
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [NestSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    } else {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [NestSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated {
    ...
    CGFloat indicatorWidth = [NestSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
